...
''' </summary>
Public Class ListViewModelBaseLight(Of dataitem As IHasID, datacontrollerclass As IDataControllerLight(Of dataitem), viewmodelitem As ItemViewModelBase(Of dataitem))
...
#Region "Methods"
    ...
    End Sub
    Public Sub AddItemToList(dataitem As dataitem, saveitem As Boolean)
        If saveitem Then
            dataitem = _DataController.CreateNewItem(dataitem)
        End If
        Dim viewmdltm = DataToItem(dataitem)
        _OriginalItemList.Insert(0, viewmdltm)
        SelectedItem = viewmdltm
        ApplyFilter()
        RaiseEvent ItemAdded(viewmdltm)
    End Sub
    ...
    End Sub
    Public Sub MoveItemToPosition(item As viewmodelitem, position As Integer)
        ItemList.Move(ItemList.IndexOf(item), position)
    End Sub
    ''' <summary>
...
#Region "Crud"
    ...
    End Function
    Public Overridable Sub CreateSecondaryItems(itemid As Integer)
    End Sub
    ...
    Private Sub ExecuteOpenNewForm()
        ...
        If Not IsNothing(dataitem) Then
            _Refresh = False
            dataitem = _DataController.CreateNewItem(dataitem)
            CreateSecondaryItems(dataitem.ID)
    ...
    Protected Overrides Sub GetData()
        GetDataAsync()
    End Sub
    Protected Async Sub GetDataAsync()
        DataLoaded = False
        IsBusy = True
        CancelLoading()
        ClearListLight()
